...
public class BinarySearchTree<E extends Comparable<E>>
    ...
    public Iterator<E> iterator() {
        Iterator<E> it = new Iterator<E>() {
            ...
            Node<E> lastPoint = null;
            Node<E> tempMin = null;

            private Node<E> start(Node<E> node){
                Node<E> temp = null;
                if(node == null)
                    return null;
                if(node.left != null)
                    temp = start(node.left);

                if( lastPoint == null && temp != null) {
                    lastPoint = temp;
                    tempMin = temp;
                }
                return node;
            ...
            public boolean hasNext() {
                if(lastPoint == null && BinarySearchTree.super.root != null)
                    return true;
                else if(lastPoint == null && BinarySearchTree.super.root == null){
                    return false;
                }
                else {
                    Node<E> temp = BinarySearchTree.super.root;
                    while (temp.right != null) {
                        temp = temp.right;
                    }
                    return temp != lastPoint;
                }
            }
            ...
            public E next() {
                if(lastPoint == null)
                    start(BinarySearchTree.super.root);
                else {
                    tempMin = null;
                    findNext(BinarySearchTree.super.root, lastPoint.data);
                    lastPoint = tempMin;
                }
            ...
            private Node<E> findNext(Node<E> localNode,E target){
                ...
                if(localNode.left != null)
                    left = findNext(localNode.left, target);

                if (localNode.data != null) {
                    if (target.compareTo(localNode.data) == -1) {
                        if (tempMin == null)
                            tempMin = localNode;
                        if (tempMin.data.compareTo(localNode.data) >= 0)
                            tempMin = localNode;
                    }
                }

                if(localNode.right != null)
                    right = findNext(localNode.right, target);
